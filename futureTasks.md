...
| Dependency majors held back | P2 | ⬜ | Four Dependabot pull requests were closed rather than merged on 2026-09-20, and it will offer them again. **next 16**: ruled out by the brief. **typescript 7**: `npm ci` cannot resolve it — typescript-eslint 8.70.0 declares `peer typescript >=4.8.4 <6.1.0`, so it needs a typescript-eslint that supports TS 7 first. **@eslint/js 10**: declares `peer eslint ^10`, so it goes with an ESLint 10 bump, not alone. **@types/node 26**: the types should describe the oldest Node this project supports (22.5), or the compiler stops catching calls that CI's Node 22 cannot run. |
